...
export default function QuizDesignPage({ onBack, onGenerate }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0b110e] text-ink">
      {/* Top Header */}
      <div className="max-w-4xl w-full mx-auto px-6 pt-12 pb-6">
        <div className="flex justify-between items-start mb-10">
          <div className="flex-1 max-w-xl">
             <input 
                ...
                className="text-4xl font-black bg-transparent border-none outline-none w-full placeholder-emerald-900 text-primary truncate mb-2"
          ...
          <div className="flex items-center bg-[#151c19] rounded-full p-1 border border-[#1e2924]">
             ...
             <button 
               ...
               className="px-6 py-2.5 rounded-full text-slate-400 font-bold text-xs uppercase tracking-wider hover:text-ink transition"
             >
               Spreadsheet Upload
             </button>
          </div>
        ...
        <div className="space-y-6">
          {questions.map((q, qIndex) => {
            ...
            return (
              <div key={q.id} className="relative bg-[#111714] border border-[#1e2924] rounded-md p-6 group">
                ...
                <div className="flex justify-between items-center mb-6 pl-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-bold tracking-widest text-primary uppercase">Primary Cognition Node</span>
                  </div>
                  <button 
                ...
                <div className="pl-4 space-y-6">
                  <div>
                    ...
                    <input 
                      type="text"
                      className="w-full bg-[#151c19] border border-[#1e2924] rounded-xl px-4 py-3 outline-none text-sm text-slate-200 focus:border-primary/50 transition placeholder-slate-600"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {['A', 'B', 'C', 'D'].map((letter, optIdx) => (
                      <div key={optIdx} className="flex flex-col">
                        ...
                        <div className="flex items-center gap-3">
                          <button 
                            className={`w-5 h-5 rounded-full border-2 flex-shrink-0 transition-colors ${q.correctAnswer === optIdx ? 'border-primary bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.5)]' : 'border-slate-600 hover:border-slate-400 bg-transparent'}`}
                            onClick={() => handleSetCorrectAnswer(q.id, optIdx)}
                          />
                          <input 
                            type="text"
                            className="w-full bg-[#151c19] border border-[#1e2924] rounded-xl px-4 py-2.5 outline-none text-sm text-slate-300 focus:border-primary/50 transition placeholder-slate-600"
        ...
        <div className="mt-8 flex justify-between items-center pb-32">
          <button 
            ...
            className="flex items-center gap-2 px-5 py-2.5 rounded-full border border-[#1e2924] hover:bg-[#151c19] text-primary text-xs font-bold tracking-widest uppercase transition"
          >
      ...
      <div className="fixed bottom-0 left-0 right-0 border-t border-[#1e2924] bg-[#0c110e]/95 backdrop-blur px-8 py-5 flex items-center justify-between z-40">
         <div className="flex items-center gap-6">
            <button onClick={onBack} className="text-slate-400 hover:text-ink transition text-xs font-bold uppercase tracking-wider">
              Cancel
            </button>
